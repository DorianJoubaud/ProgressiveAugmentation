...
for i in range(len(folders)):
    ...
    for i in range(3):
        taccu,tmcc, tf, trec, tpres, tg = raw_data(dataset, x_train, y_train, x_test,y_test,  np.argmax(y_test, axis = 1), input_shape,  nb_class)
    ...
    for i in range(3):
        taccu,tmcc, tf, trec, tpres, tg = ROS_test(dataset, x_train, y_train, x_test,y_test,  np.argmax(y_test, axis = 1), input_shape,  nb_class)
    ...
    for i in range(3):
        taccu,tmcc, tf, trec, tpres, tg = jitter_test(dataset, x_train, y_train, x_test,y_test,  np.argmax(y_test, axis = 1), input_shape,  nb_class, rat, majority_class)
    ...
    for i in range(3):
        taccu,tmcc, tf, trec, tpres, tg = tw_test(dataset, x_train, y_train, x_test,y_test,  np.argmax(y_test, axis = 1), input_shape,  nb_class, rat, majority_class)
    ...
    for i in range(3):
        taccu,tmcc, tf, trec, tpres, tg = SMOTE_test(dataset, x_train, y_train, x_test,y_test,  np.argmax(y_test, axis = 1), input_shape,  nb_class)
    ...
    for i in range(3):
        taccu,tmcc, tf, trec, tpres, tg = ADASYN_test(dataset, x_train, y_train, x_test,y_test,  np.argmax(y_test, axis = 1), input_shape,  nb_class)
...
acc = pd.DataFrame(evolutiona)
acc = acc.rename(columns={0 : 'ROS', 1:'Jittering', 2:'Time Warping', 3:'SMOTE', 4:'ADASYN'})
acc = acc.rename(index={i : folders[i] for i in range(len(folders))})
acc.to_csv('Results/Accuracy/acc.csv')

mcc = pd.DataFrame(evolutionmcc)
mcc = mcc.rename(columns={0 : 'ROS', 1:'Jittering', 2:'Time Warping', 3:'SMOTE', 4:'ADASYN'})
mcc = mcc.rename(index={i : folders[i] for i in range(len(folders))})
mcc.to_csv('Results/MCC/mcc.csv')


f = pd.DataFrame(evolutionf)
f= f.rename(columns={0 : 'ROS', 1:'Jittering', 2:'Time Warping', 3:'SMOTE', 4:'ADASYN'})
f = f.rename(index={i : folders[i] for i in range(len(folders))})
f.to_csv('Results/F1_scores/f1.csv')

g = pd.DataFrame(evolutiong)
g = g.rename(columns={0 : 'ROS', 1:'Jittering', 2:'Time Warping', 3:'SMOTE', 4:'ADASYN'})
g = g.rename(index={i : folders[i] for i in range(len(folders))})
g.to_csv('Results/G_scores/g.csv')

p= pd.DataFrame(evolutionpres)
p= p.rename(columns={0 : 'ROS', 1:'Jittering', 2:'Time Warping', 3:'SMOTE', 4:'ADASYN'})
p = p.rename(index={i : folders[i] for i in range(len(folders))})
p.to_csv('Results/Precision/pres.csv')

r= pd.DataFrame(evolutionrec)
r= r.rename(columns={0 : 'ROS', 1:'Jittering', 2:'Time Warping', 3:'SMOTE', 4:'ADASYN'})
