# -*- coding: utf-8 -*-
"""
Created on Wed May 18 09:06:22 2022
...
def raw_data(dataset, x_train, y_train, x_test, y_test, input_shape, nb_classes):
    ...
    return accu
def ROS_test(dataset, x_train, y_train, x_test,  y_test, input_shape,  nb_classes):
...
def jitter_test(dataset, x_train, y_train, x_test,  y_test, input_shape,  nb_classes, rat, majority_class):
    ...
    for i in range(len(rat)):
          if (rat[i] != rat[majority_class]):
            
            if tilt == 0:
              
    
              aug, aug_labels = Augmentation(jitter,x_train, y_train,i ,rat[majority_class]-rat[i])
              
              tilt = 1
            else:
    
              tmp_data, tmp_labels = Augmentation(jitter,x_train, y_train,i ,rat[majority_class]-rat[i])
    ...
    model.build_model(input_shape, nb_classes)
    model.fit(oversamp, oversamp_labels)
    accu = model.evaluate(x_test,y_test)
    return accu
    
def tw_test(dataset, x_train, y_train, x_test,  y_test, input_shape,  nb_classes, rat, majority_class):
    def Augmentation(function, data, label_data, class_under, nb):
      underReprClass = list()
    ...
    for i in range(len(rat)):
        if (rat[i] != rat[majority_class]):
            ...
            if tilt == 0:
              ...
              aug, aug_labels = Augmentation(time_warp,x_train, y_train,i ,rat[majority_class]-rat[i])
              tilt = 1
            else:
    
              tmp_data, tmp_labels = Augmentation(time_warp,x_train, y_train,i ,rat[majority_class]-rat[i])
