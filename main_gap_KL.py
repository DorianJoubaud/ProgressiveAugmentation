...
for i in range(len(folders)):
    ...
    y_majority_idxs = np.where(y_train == majority_class)[0]
    ...
    tmp_accu = 0
    for i in range(3):
        accu = SMOTE_test(dataset, x_train_new, y_train_new, x_test,  y_test, input_shape,  nb_class)
        tmp_accu += accu
    accu = tmp_accu/3
    ...
    while(add_nb != total_add_nb):
      ...
      for i in sorted_rat_next: 
        ...
        if (new_rat[i] != new_rat[majority_class]):
          ...
          accu = 0
          mcc = 0
          f = np.array([0 for cl in nb_class])
          rec = np.array([0 for cl in nb_class])
          pres = np.array([0 for cl in nb_class])
          g = np.array([0 for cl in nb_class])
          


          for ite in range(3):
            taccu,tmcc, tf, trec, tpres, tg += SMOTE_test(dataset, x_train_new, y_train_new, x_test,  y_test, input_shape,  nb_class, sp_strg)
            print(tf)
