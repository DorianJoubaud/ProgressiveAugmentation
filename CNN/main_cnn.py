...
for i in range(len(folders)):
    ...
    while(add_nb != total_add_nb):
      ...
      for i in sorted_rat_next:
        ...
        if (new_rat[i] != new_rat[majority_class]):
          ...
          for ite in range(3):
            taccu,tmcc, tf, trec, tpres, tg = tw_test(dataset, x_train_new, y_train_new, x_test,  np.argmax(y_test, axis = 1), input_shape,  nb_class, sp_strg)
    ...
    for i in range(3):
        taccu,tmcc, tf, trec, tpres, tg = tw_test(dataset, x_train_new, y_train_new, x_test,  np.argmax(y_test, axis = 1), input_shape,  nb_class, full_sp)
